...
class SindhiTextSegmenter:
    ...
    def process(self, segment_level="words", save_debug=True):
        ...
        output_dir = Path(self.output_dir) / "words" # e.g., "segmented_output/ligatures"
        output_images = list(output_dir.glob("*.png"))  # or "*.jpg" if JPG

        if not output_images:
            print(f"[⚠️ Warning] No images found in {output_dir}. Check if segmentation step saved correctly.")

        return [str(p) for p in output_images]
